...
- [x] **#FL-007** [P1] Replace urllib.request with httpx in libs/llm_client.py — proper timeout + connection pooling ✅ DONE 2026-03-13 (done file confirmed, commit 797c3ae9)
- [x] **#FL-008** [P1] Expand easy_button test coverage: DashboardView, ClientsView, SignalsView, ActionsView
- [ ] **#FL-009** [P1] Generate OpenAPI schema for easy_button endpoints via drf-yasg ⏳ Agent spawned 2026-03-13 16:07 UTC
- [ ] **#FL-010** [P1] Upgrade django-celery-beat from 2.2.* to >=2.6,<3.0
- [ ] **#FL-011** [P1] Raise coverage gate from 50% to 70% in tox.ini
- [x] **#FL-012** [P1] Add LLM client unit tests (mock httpx, test Gemini→Kimi fallback chain) ✅ VERIFIED 2026-03-13 — test file exists at easy_button/tests/test_llm_client.py (11 tests covering call_gemini/call_kimi/call_with_fallback, mock httpx, JSON parsing, fallback chain)
...
- [x] **#894** [P1] Vitest unit test setup (validateContact, ROI math, MDX parsing) [3h] → ✅ DONE 2026-03-10 (partial — Vitest configured + roi.ts 19 tests; validateContact + MDX parsing tests pending) commit 6b15b1d
- [x] **#899** [P1] Extract ROI calculate() to src/lib/roi.ts (prereq for testing) [30min] → ✅ DONE 2026-03-10 — src/lib/roi.ts created, ROICalculator.tsx imports from it, 19 Vitest tests pass, commit 6b15b1d
...
- [x] [P1/S] #838 Fix X-Forwarded-For spoofing — add TRUSTED_PROXY_IPS validation → `838-pending-high-signal-studio-auth-trusted-proxy-ips.md`
...
- [x] [P0/S] #SBB-002: Replace jsonpickle with standard json for SQL params — `apps/signals/schemas/signal.py:112,129` ✅ DONE 2026-03-10 — _SafeSQLParamsEncoder replaces jsonpickle, json.loads replaces jsonpickle.decode (RCE eliminated), commit 16fbd18
- [x] [P0/M] #SBB-003: Add multi-tenant isolation tests (verify org A cannot access org B data/signals) ✅ DONE 2026-03-10 (done file confirmed)
...
### P0 — Revenue Critical
- [x] [P0/M] #NR-001 Add quota enforcement middleware — block requests when used_tokens >= quota_tokens, HTTP 429 + upgrade_url ✅ DONE 2026-03-10 — smartAuth middleware added to tenantAuth.ts (tenant key with quota guard OR admin key bypass), wired to /api/pipeline routes in index.ts, incrementUsage() called after /generate + /research. Commit 305dcef, pushed GitHub main.
- [x] [P0/XS] #NR-003 Add /api/billing/usage self-serve endpoint (used/limit/reset_at/plan) ✅ VERIFIED 2026-03-10 — already implemented at billing.ts:156 via tenantAuth middleware. No changes needed.
...
- [x] [P1/M] #NR-010 Supertest E2E test suite for full Express HTTP stack (auth, rate limit, billing, scheduler) → Initial E2E test `src/tests/e2e/billing.test.ts` created by Honey 2026-03-14. Covers /api/billing/usage for auth + structure. [expand coverage in future runs]
...
- [x] [P1/S] #NR-022 Centralize SQLite DB opens into shared src/lib/db.ts singleton → DONE (added busy_timeout=5000 pragma; confirmed WAL+FK+DATABASE_PATH env already in place; all services/routes use getDb() singleton; pushed commit 96d99df)
...
- [x] [P0/M] #FME-003 ✅ DONE 2026-03-09 (commit 797d28f) — billing.ts wired to real Supabase checkout + credit_transactions
